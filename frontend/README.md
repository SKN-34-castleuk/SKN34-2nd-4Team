...
- 로그인 후 부서별 페이지 이동
- 실제 아이디 기억하기
- 회원가입 페이지와 회원가입 API 호출

현재 FastAPI에는 인증 API가 없습니다. 따라서 유효한 아이디와 비밀번호를
입력하고 로그인 버튼을 누르면 실제 로그인을 수행하지 않고 다음 안내만
표시합니다.
...
회원가입 버튼도 현재는 UI만 있으며 클릭 동작은 연결되어 있지 않습니다.
...
실제 인증을 구현할 때는 `handleSubmit`의 마지막 안내 메시지 부분을 인증 API
호출로 교체하게 됩니다.
...
현재 로그인과 회원가입에 대응하는 FastAPI API는 없습니다. 실제 기능을
연결하려면 로그인, 회원가입, 로그아웃, 현재 사용자 조회 API를 먼저 설계해야
합니다.
...
- 실제 인증을 구현할 때는 토큰 저장 방식과 만료 정책을 먼저 결정합니다.
...
1. FastAPI 로그인·회원가입 API 명세 작성
2. 사용자 역할 정의: 관리자, 분석, 운영, 마케팅
3. 로그인 요청과 오류 응답 연결
4. 인증 토큰 저장·갱신·만료 정책 구현
5. React Router 추가
6. 로그인 후 역할별 화면 이동
7. 로그아웃과 현재 사용자 조회 구현
8. 예측 입력 화면과 `/api/v1/predictions` 연결
