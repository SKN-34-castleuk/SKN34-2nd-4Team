...
현재 Backend 테스트는 23개이며 다음을 포함합니다.
...
현재 단계는 DB에 저장할 분석 스냅샷을 완성한 단계입니다. 다음 구현은
\`customer_insights\` 최신 결과와 이력 조회 API, 고위험 고객 화면, 캠페인 대상
생성·처리 API, 역할별 권한 제어 순서로 진행합니다.
